<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=7F3ACE&height=120&section=header"/>

<h1 align="center">Ali Mohammadi</h1>

<p align="center">
Cybersecurity • DevOps • Systems Engineering
</p>

<p align="center">
  <img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&duration=3000&pause=800&color=7F3ACE&center=true&vCenter=true&width=720&lines=Cybersecurity+%7C+DevOps+%7C+Systems;Building+Secure+Systems;Infrastructure+%26+Automation;Backend+Engineering" />
</p>

---

## 🛡 About Me

Computer Engineering student at **Shahid Beheshti University**  
GPA: **18.54 / 20**  
DevOps Team Member – Faculty of Computer Science  

Focused on designing and understanding systems from low-level fundamentals to secure deployment pipelines.

**Primary Interests**
- Cybersecurity & Secure Architecture
- Networking & Infrastructure
- Backend Systems
- DevOps & Automation
- Systems Programming

---

## ⚔️ Technical Arsenal

**Languages:** C • C++ • Java • Python • Bash  
**Backend:** Django  
**DevOps & Infrastructure:** Docker • Kubernetes • CI/CD • GitHub Actions  
**Systems:** Linux • Networking Fundamentals  
**Certifications:** Network+ Certified  

---

## 🚀 Selected Projects

- 🔧 [Compiler Project](https://github.com/mly808299/compiler) — Language processing and compiler design fundamentals  
- 🌐 [Django Blog Platform](https://github.com/mly808299/blog_site) — Modular backend with authentication and clean architecture  
- 🎓 [Student Platform – Java & Flutter](https://github.com/mly808299/AMRock_Mafi) — Java backend + Flutter frontend (OOP-driven)  
- 📡 [V2Ray Infrastructure Tooling](https://github.com/SBU-DCAI/v2ray-vpn) — Containerized VPN deployment setup  
- 🤖 [vLLM Optimization](https://github.com/eagertowork/vLLM_Optimization) — LLM performance experimentation and infra testing  

---

## 📊 GitHub Analytics (Self-hosted)

<p align="center">
  <img src="https://raw.githubusercontent.com/mly808299/mly808299/main/profile-summary-card-output/midnight_purple/0-profile-details.svg" />
</p>

<p align="center">
  <img src="https://raw.githubusercontent.com/mly808299/mly808299/main/profile-summary-card-output/midnight_purple/3-stats.svg" height="165"/>
  <img src="https://raw.githubusercontent.com/mly808299/mly808299/main/profile-summary-card-output/midnight_purple/1-repos-per-language.svg" height="165"/>
</p>

<p align="center">
  <img src="https://raw.githubusercontent.com/mly808299/mly808299/main/profile-summary-card-output/midnight_purple/2-most-commit-language.svg" height="165"/>
  <img src="https://raw.githubusercontent.com/mly808299/mly808299/main/profile-summary-card-output/midnight_purple/4-productive-time.svg" height="165"/>
</p>


---

## 🐍 Contribution Graph

<p align="center">
  <img src="https://raw.githubusercontent.com/mly808299/mly808299/output/github-contribution-grid-snake-dark.svg?v=1" />
</p>

---

## 🌍 Connect

📧 ali.mohammadi121wk@gmail.com  
🔗 [LinkedIn](https://www.linkedin.com/in/ali-mohammadi-bb2872330/)  

---

> Engineering discipline. Security mindset. Continuous improvement.

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=7F3ACE&height=120&section=footer"/>
